...
///
/// `Send` を要求する（[`DeviceWatcher`] をスレッド間で渡せるようにするため）。
/// PipeWire のような `!Send` 実装は内部で専用スレッドへ閉じ込め、本体は `Send` な
/// ハンドルだけ持つこと（`PwDeviceWatcher` がそうしている）。
...
/// はこれへ縮退して `Ok` を返す（着脱監視は変化が来なければ no-op で良い。
/// `devices()` がデーモン不在を空リストに握るのと一貫）。
struct NoopWatcher;
...
//
// 孤児ルール回避: trait は facade（このクレート）、型は flexaudio-os-linux だが、
// **trait をローカル所有する**ので facade 内 impl が許される。os-linux は core
// にしか依存しない（facade の trait を知らない）構造を保ったまま、ここで橋渡しする。
#[cfg(target_os = "linux")]
...
///
/// - **Linux**: `PwDeviceWatcher::start()`（PipeWire 永続監視）を試み、成功すれば
///   それを包む。**失敗（PipeWire 不在等）なら [`NoopWatcher`] へ縮退して `Ok`**
///   を返す（`devices()` がデーモン不在を空に握るのと一貫＝着脱監視は変化が来なければ
///   no-op で良い）。
...
mod tests {
    ...
    /// `watch_devices()` は PipeWire 不在の homelab でも **panic せず**
