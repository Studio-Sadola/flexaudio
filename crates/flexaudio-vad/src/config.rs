//! VAD 設定。値はすべて silero-VAD 原典 (`get_speech_timestamps`) の正準デフォルトに準拠する。

/// VAD の挙動を制御する設定。
///
/// デフォルト値は silero-VAD の `get_speech_timestamps` 正準値に一致する。
#[derive(Debug, Clone, PartialEq)]
pub struct VadConfig {
    /// 発話開始とみなす確率しきい値 (>=)。既定 0.5。
    pub threshold: f32,
    /// 無音開始とみなす負側しきい値 (<)。`None` の場合は
    /// `max(threshold - 0.15, 0.01)` を用いる (silero 準拠)。
    pub neg_threshold: Option<f32>,
    /// 採用する発話の最小長 (ms)。これ未満のセグメントは破棄。既定 250。
    pub min_speech_ms: u32,
    /// 発話終了を確定するのに必要な無音長 (ms)。既定 100 (silero 正準)。
    pub min_silence_ms: u32,
    /// セグメント境界を前後に拡張するパディング (ms)。既定 30 (silero 正準)。
    pub speech_pad_ms: u32,
    /// 1 セグメントの最大長 (ms)。0 = 無制限。超過時は強制分割。既定 0。
    pub max_speech_ms: u32,
    /// サンプルレート。8000 または 16000 のみ許容。既定 16000。
    pub sample_rate: u32,
}

impl Default for VadConfig {
    /// silero 正準デフォルト (= [`VadConfig::balanced`])。
    fn default() -> Self {
        VadConfig {
            threshold: 0.5,
            neg_threshold: None,
...
impl VadConfig {
    ...
    /// バランス型プリセット。silero 正準デフォルトと同一 ([`VadConfig::default`])。
    ...
    /// 実効的な負側しきい値を返す。
    ///
    /// 明示指定があればそれを、なければ `max(threshold - 0.15, 0.01)` (silero 準拠) を返す。
